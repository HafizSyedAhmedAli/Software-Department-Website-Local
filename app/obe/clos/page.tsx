...
export const metadata: Metadata = {
  ...
  description: "Course Learning Outcomes for all Software Engineering courses at QUEST Nawabshah.",
};
  ...
  return (
    <>
      <PageHeader
        ...
        crumbs={[{ label: "OBE", href: "#" }, { label: "CLOs" }]}
      />
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <p className="font-body text-slate-500 leading-relaxed mb-12 max-w-3xl">
            Each Course Learning Outcome (CLO) is mapped to a corresponding Program Learning Outcome
            (PLO) and classified by learning domain and Bloom&apos;s Taxonomy level to ensure
            comprehensive coverage of graduate attributes.
          </p>

          <div className="space-y-10">
            {COURSES_DATA.map((course) => (
              <div key={course.code} className="border border-slate-100 rounded-sm overflow-hidden shadow-card">
                {/* Course header */}
                    ...
                    <tbody>
                      {course.clos.map((clo) => (
                        <tr key={clo.clo}>
                          ...
                          <td className="text-sm leading-relaxed">{clo.description}</td>
                          <td>
                            <span
                              className={`text-xs font-semibold px-2 py-0.5 rounded-sm border ${
                                DOMAIN_COLORS[clo.domain] ?? "bg-slate-50 text-slate-600 border-slate-200"
                              }`}
                            >
                              {clo.domain}
                            </span>
          ...
          <div className="mt-12 bg-slate-50 border border-slate-100 rounded-sm p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs font-body text-slate-600">
              <div>
                <p className="font-semibold text-navy-800 mb-1">Cognitive (C)</p>
                <p>C1: Remember · C2: Understand · C3: Apply</p>
                <p>C4: Analyse · C5: Evaluate · C6: Create</p>
              </div>
              <div>
                <p className="font-semibold text-navy-800 mb-1">Psychomotor (P)</p>
                <p>P1: Imitation · P2: Manipulation · P3: Precision</p>
                <p>P4: Articulation · P5: Naturalisation</p>
              </div>
              <div>
                <p className="font-semibold text-navy-800 mb-1">Affective (A)</p>
