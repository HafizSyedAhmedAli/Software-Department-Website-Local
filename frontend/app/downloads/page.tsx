...
export default async function DownloadsPage() {
  const downloads = await api.downloads().catch(() => DOWNLOADS_DATA);
            ...
            <table className="dept-table">
              ...
              <tbody>
                {downloads.map((item, i) => (
                  <tr key={item._id ?? item.fileUrl}>
                    <td className="text-slate-400 font-mono text-center">
                      {i + 1}
                    </td>
                    <td>
                    ...
                    <td>
                      <span
                        className={`text-xs font-mono font-semibold px-2 py-0.5 rounded-sm border ${FILE_BADGE[item.fileType] ?? "bg-slate-50 text-slate-600 border-slate-200"}`}
                      >
                        {item.fileType}
                      </span>
                    </td>
