...
export const metadata: Metadata = { title: "Course Learning Outcomes (CLOs)" };
...
export default async function CLOsPage() {
  const courses = await api.courses().catch(() => COURSES_DATA);
  const withCLOs = courses.filter((c) => c.clos?.length);

  return (
    <>
      <PageHeader
        title="Course Learning Outcomes"
        subtitle="Per-course CLOs mapped to PLOs with Bloom's Taxonomy levels."
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <p className="font-body text-slate-500 leading-relaxed mb-12 max-w-3xl">
            Each CLO is mapped to a PLO and classified by learning domain and
                    ...
                    <tbody>
                      {course.clos.map((clo) => (
                        <tr key={clo.clo}>
                          ...
                          <td>
                            <span
                              className={`text-xs font-semibold px-2 py-0.5 rounded-sm border ${DOMAIN_COLORS[clo.domain] ?? "bg-slate-50 text-slate-600 border-slate-200"}`}
                            >
                              {clo.domain}
                            </span>
                          </td>
