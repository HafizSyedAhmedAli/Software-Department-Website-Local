    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="absolute top-full left-0 mt-2 w-56 bg-white border border-slate-100 shadow-xl rounded-sm z-50 py-1 overflow-hidden"
        >
          {items.map((item, i) => (
            <motion.div
              key={item.href}
            ...
            >
              <Link
                href={item.href}
                className="group flex items-center gap-2 px-4 py-2.5 text-sm font-body text-slate-700 hover:bg-navy-950 hover:text-white transition-all duration-150"
              >
...
function DesktopNavLink({ item, active }: { item: NavItem; active: boolean }) {
  ...
  if (item.children) {
    return (
      ...
      >
        <button
          className={clsx(
            "flex items-center gap-1.5 px-3 py-2 text-sm font-body font-medium rounded-sm transition-colors duration-200 relative group",
  ...
  return (
    <Link
      href={item.href}
      className={clsx(
        "relative px-3 py-2 text-sm font-body font-medium rounded-sm transition-colors duration-200 group",
        active
  ...
  return (
    <>
      ...
      <motion.nav
        ...
        animate={{
          boxShadow: scrolled
            ? "0 2px 20px rgba(0,23,48,0.12)"
            : "0 1px 3px rgba(0,23,48,0.06)",
        }}
      >
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <Link href="/" className="flex items-center gap-3 shrink-0">
              <motion.div
                className="relative w-40 h-10 md:w-64 md:h-16"
